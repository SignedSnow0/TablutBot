...
/*
 * Calculates the total number of unobstructed, straight-line paths the King
 * currently has to the edges of the board.
 */
uint8_t EscapeRoutes(const Tablut &t, const PiecePosition &king);

/**
 * Counts how many enemy Mercenaries (Black pieces) are in the immediate
...
/**
 * Counts how many Guards (White pieces) are in the immediate vicinity (3 x 3
 * grid centered in the king) of the King.
 */
uint8_t GuardsAdjacentKing(const Tablut &t, const PiecePosition &king);

/**
 * Calculates the minimum number of moves required for a piece (usually the
 * King) to escape to any edge of the board.
...
/**
 * Evaluates the overall positional strength of the White pieces (Guards)
 * It uses a weighted sum of the positions of the pieces
 */
int64_t PositionWeigthedWhite(const Tablut &t);

/**
 * Evaluates the overall positional strength of the King
 * It uses a weighted sum of the positions of the pieces
 */
int64_t PositionWeigthedKing(const Tablut &t);

/**
 * Evaluates the overall positional strength of the Black pieces (Mercenaries)
 * It uses a weighted sum of the positions of the pieces
 */
